...
if !frozen {
    ...
    v_input = input_check(key_down);
    if v_input == 0 {
        v_input = -input_check(key_up);
    }

    if room == rParty && global.hamster {
        if irandom(120) == 0
            { global.hHamster = -global.hHamster; }

        h_input *= global.hHamster;
    }
}
...
if global.gradius {
    sprite_index = sprGradius;
    image_speed = 0;
    exit;
}

if on_floor || place_meeting(x,y,objPYFPlatformYosniper) {
    if h_input != 0 {
        player_set_skin("Run");//sprite_index = sprPlayerRun;
        image_speed = 0.5;
    }
    else {
        player_set_skin("Idle");//sprite_index = sprPlayerIdle;
        image_speed = 0.2;
    }
}
else if vine_direction != 0 {
    player_set_skin("Slide");//sprite_index = sprPlayerSlide;
    image_speed = 0.5;
}
else {
    if vspeed * global.grav < 0 {
        player_set_skin("Jump");//sprite_index = sprPlayerJump;
        image_speed = 0.5;
    }
    else {
        player_set_skin("Fall");//sprite_index = sprPlayerFall;
        image_speed = 0.5;
    }
}

//Gamblecore
if (config_get("player_skin") == 5){
    image_speed = 1/3;
}

//Play online
xs = abs(image_xscale) * x_scale;
#define Other_4
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
...
// gradius gimmick
if global.gradius && input_check(key_jump) {
    draw_sprite_ext(mask_index, 0, _draw_x, _draw_y, 1, 1, 0, c_white, 0.5);
}

draw_sprite_ext(mask_index, image_index, _draw_x, _draw_y, x_scale, image_yscale, image_angle, _draw_color, 0.5*global.debug_show_mask);

// Draw the bow
if has_bow {
    draw_sprite_ext(sprPlayerBow, image_index, _draw_x, _draw_y + global.gradius*6, x_scale, image_yscale * global.grav, image_angle, image_blend, image_alpha);
}
